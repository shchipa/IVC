...
<table border='0' width="100%" cellpadding="0" cellspacing="0">
	<tr valign='center'>
		...
		<?
			printf ("<td align='right' height='50' width='130'><a href='remont_print_v_remonte.php?text=%s'><span class='p2'>форма для печати</span></a></td>", $text);
		?>
	</tr>
</table>
		...
		<td>
			<div>
   				<select name="sort1" onChange="top.location.href = this.options[this.selectedIndex].value">
                    ...
                    <?php
						$result = mysql_query('SELECT sort FROM sort2',$db);
						$mysort = mysql_fetch_array($result);
						do
						{
							if (isset($sort1) && $mysort['sort'] == $sort1)
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s' selected>%s</option>", $sort1, $sort2, $sort3, $sort4, $sort1);
							}
							else
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s'>%s",$mysort['sort'], $sort2, $sort3, $sort4, $mysort['sort']);
							}
						}
   				...
   				<select name="sort2" onChange="top.location.href = this.options[this.selectedIndex].value">
                    ...
                    <?php
						...
						do
						{
							...
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s' selected>%s</option>", $sort1, $sort2, $sort3, $sort4, $sort2);
							}
							else
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s'>%s", $sort1, $mysort['sort'], $sort3, $sort4, $mysort['sort']);
							}
						}
							while ($mysort = mysql_fetch_array($result));
   			     	?>
   				...
   				<select name="sort3" onChange="top.location.href = this.options[this.selectedIndex].value">
                    ...
                    <?php
						...
						do
						{
							...
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s' selected>%s</option>", $sort1, $sort2, $sort3, $sort4, $sort3);
							}
							else
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s'>%s", $sort1, $sort2, $mysort['sort'], $sort4, $mysort['sort']);
							}
						}
							while ($mysort = mysql_fetch_array($result));
   			     	?>
   				...
   				<select name="sort3" onChange="top.location.href = this.options[this.selectedIndex].value">
                    ...
                    <?php
						...
						do
						{
							...
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s' selected>%s</option>", $sort1, $sort2, $sort3, $sort4, $sort4);
							}
							else
							{
								printf ("<option value='remont_v_remonte.php?sort1=%s&sort2=%s&sort3=%s&sort4=%s'>%s", $sort1, $sort2, $sort3, $mysort['sort'], $mysort['sort']);
							}
						}
							while ($mysort = mysql_fetch_array($result));
   			     	?>
...
<?
	printf("
		...
		");
			...
			$resav = mysql_query ("SELECT * FROM remont WHERE sost = 'в ремонте' ORDER BY $text",$db);
